...
async def get_distances(
    ...
    group: str = Query(
        ..., description="The group to get the distances for (try 'samples')"
    ),
    sample_id: List[str] = Query(
        ...,
        description="The list of samples to get the distances for (need at least 3 samples)",
    ),
):
    if group == "samples":
        curr = db.sample.find({"sample_id": {"$in": sample_id}})
    ...
    for sample in samples:
        ...
        date = time.strftime(
            "%Y-%m-%d", time.localtime(sample.get("collection_date")["$date"])
        )
    ...
    for key in tree["metadata"]:
        if len(tree["metadata"][key]) > length:
            longest_obj = key
