...
async def post_user(user: User, current_user: User = Depends(get_current_active_user)):
    if current_user["scope"] != "admin":
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
async def delete_user(user: str, current_user: User = Depends(get_current_active_user)):
    if user == "admin":
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
