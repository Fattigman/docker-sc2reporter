...
async def startup_db():
    ...
    if 'users' not in collections:
        ...
        hashed_password = CryptContext(schemes=["bcrypt"], deprecated="auto").hash(settings.ADMIN_PASSWORD)
        collection = db.users
        try:
            collection.insert_one({"_id":settings.ADMIN_USERNAME,
    ...
    if 'significant_variants' not in collections:
        ...
        collection.insert_one({"_id": "significant_variants", 
