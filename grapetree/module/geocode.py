...
def geoCoding_openstreet(location, country="", countries=None):
    ...
    if len(data) > 0:
            ...
            if d1["importance"] > 0:
                ...
                for jd in range(id + 1, len(data)):
                    ...
                    if d2["importance"] > 0:
                        ...
                        if (
                            (e1 - s1 + 1) >= 0.9 * (db1[1] - db1[0] + 1)
                            or (e1 - s1 + 1) >= 0.9 * (db2[1] - db2[0] + 1)
                        ) and (
                            (e2 - s2 + 1) >= 0.9 * (db1[3] - db1[2] + 1)
                            or (e2 - s2 + 1) >= 0.9 * (db2[3] - db2[2] + 1)
                        ):
                            if (db2[1] - db2[0] + 1) * (db2[3] - db2[2] + 1) > (
                                db1[1] - db1[0] + 1
                            ) * (db1[3] - db1[2] + 1):
                                if "city" in d1["address"] and (
                                    (
                                        "county" in d2["address"]
                                        and d1["address"]["city"]
                                        != d2["address"]["county"]
                                    )
                                    or (
                                        "county" not in d2["address"]
                                        and d1["address"]["city"]
                                        != d2["address"]["state"]
                                    )
                                ):
                                    ...
                                    d1, db1 = data[id], np.array(
                                        data[id]["boundingbox"], dtype=float
                                    )
                            elif (db2[1] - db2[0] + 1) * (db2[3] - db2[2] + 1) < (
                                db1[1] - db1[0] + 1
                            ) * (db1[3] - db1[2] + 1):
                                if "city" in d2["address"] and (
                                    (
                                        "county" in d1["address"]
                                        and d2["address"]["city"]
                                        == d1["address"]["county"]
                                    )
                                    or (
                                        "county" not in d1["address"]
                                        and d2["address"]["city"]
                                        == d1["address"]["state"]
                                    )
                                ):
        ...
        res["Longitude"], res["Latitude"], res["confidence"] = (
            data[0]["lon"],
            data[0]["lat"],
            str(data[0]["importance"]),
        )

        country2 = np.unique(
            [d["address"].get("country_code", "") for d in data], return_counts=True
        )
        if np.max(country2[1]) > 0.5 * len(data):
            country2 = countries[
                countries.T[0] == country2[0][(np.argmax(country2[1]))].upper(), 3:
            ]
            if len(country2):
                res["Country"], res["Continient"] = unidecode.unidecode(
                    country2[0, 0]
                ), unidecode.unidecode(country2[0, 1])

        state = np.unique(
            [d["address"].get("state", "") for d in data], return_counts=True
        )
        if np.max(state[1]) > 0.5 * len(data):
            res["admin1"] = state[0][np.argmax(state[1])]
        county = np.unique(
            [d["address"].get("county", "") for d in data], return_counts=True
        )
        if np.max(county[1]) > 0.5 * len(data):
            res["admin2"] = county[0][np.argmax(county[1])]
        city = np.unique(
            [d["address"].get("city", "") for d in data], return_counts=True
        )
...
def geoCoding(data):
    countries = pd.read_csv(
        os.path.join(os.path.dirname(__file__), "countries.csv"), encoding="ISO-8859-1"
    ).values
    try:
    ...
    for d in data:
        ...
        if key in history:
            ...
            r = geoCoding_openstreet(
                "+".join(
                    [l for l in re.split(r"[_\-,\s]+", location.lower()) if l != ""]
                ),
                country=country,
                countries=countries,
            )
    ...
    if isinstance(history, dict):
        np.savez_compressed(
            os.path.join(os.path.dirname(__file__), "geocode.npz"), **history
        )
    return res
