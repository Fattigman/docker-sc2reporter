...
def contemporary(a, b, c, n_loci):
    ...
    if b >= a[0] + c and b >= a[1] + c:
        ...
        return True
    s11, s12 = np.sqrt(1 - a[0] / n_loci), (2 * n_loci - b - c) / 2 / np.sqrt(
        n_loci * (n_loci - a[0])
    )
    ...
    p1 = (
        a[0] * np.log(1 - s11 * s11)
        + (n_loci - a[0]) * np.log(s11 * s11)
        + (b + c) * np.log(1 - s11 * s12)
        + (2 * n_loci - b - c) * np.log(s11 * s12)
    )
    p2 = (
        a[1] * np.log(1 - s21)
        + (n_loci - a[1]) * np.log(s21)
        + b * np.log(1 - s21 * s22)
        + (n_loci - b) * np.log(s21 * s22)
        + c * np.log(1 - s22)
        + (n_loci - c) * np.log(s22)
    )
    return p1 >= p2
...
def add_args():
    ...
    parser.add_argument(
        ...
        "-p",
        dest="fname",
        help="[REQUIRED] An input filename of a file containing MLST or SNP character data, OR a fasta file containing aligned sequences. \n",
        required=True,
    )
    parser.add_argument(
        "--method",
    ...
    parser.add_argument(
        ...
        "-r",
        dest="branch_recraft",
        help="Triggers local branch recrafting. [DEFAULT: MSTreeV2]. ",
        default=False,
        action="store_true",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--wgMLST",
        "-w",
        help="[EXPERIMENTAL] a better support of wgMLST schemes.",
        default=False,
        action="store_true",
    )
    parser.add_argument(
        ...
        "-t",
        dest="heuristic",
        help='Tiebreak heuristic used only in MSTree and MSTreeV2\n"eBurst" [DEFAULT: MSTree]\n"harmonic" [DEFAULT: MSTreeV2]',
        default="eBurst",
    )
    parser.add_argument(
        "--n_proc",
        ...
        help="Number of CPU processes in parallel use. [DEFAULT]: 5. ",
        type=int,
        default=5,
    )
    parser.add_argument(
        ...
        "-c",
        dest="checkEnv",
        help="Only calculate the expected time/memory requirements. ",
        default=False,
        action="store_true",
    )
    parser.add_argument(
        ...
        "-b",
        dest="block_penalty",
        help='[DEFAULT: 0.01] The penalty that is given to a different locus if it is led by another difference. Only works for "-x blockwise"',
        default=0.01,
    )
    ...
    if args.matrix_type == "blockwise":
        if args.method == "MSTreeV2":
            args.method = "MSTree"
        sys.stderr.write(
            'You have chosen the "blockwise" matrix. The --recraft option will be disabled and all values in the profile will be treated as real alleles\n\n'
        )
...
class distance_matrix(object):
    ...
    def get_distance(func, profiles, handle_missing):
        ...
        if n_proc > 1:
            ...
            indices = np.array(
                [
                    [n_profile * v / n_proc + 0.5, n_profile * (v + 1) / n_proc + 0.5]
                    for v in np.arange(n_proc, dtype=float)
                ],
                dtype=int,
            )
            del profiles
    ...
    def asymmetric_wgMLST(profiles, handle_missing="pair_delete", index_range=None):
        ...
        distances = np.zeros(
            shape=[profiles.shape[0], index_range[1] - index_range[0]], dtype=np.float32
        )
        ...
        if handle_missing not in ("absolute_distance",):
            for i2, id in enumerate(np.arange(*index_range)):
                ...
                diffs = (
                    np.sum(
                        ((profiles != profile) & (presences * presence))
                        + (presences < presence) * pp,
                        axis=1,
                    )
                    * float(presence.size)
                    / np.sum(presence)
    ...
    def blockwise(profiles, handle_missing=0.01, index_range=None):
        ...
        for i2, id in enumerate(np.arange(*index_range)):
            ...
            diffs = np.hstack(
                [
                    np.zeros([profiles.shape[0], 1], dtype=int),
                    profiles - profile,
                    np.zeros([profiles.shape[0], 1], dtype=int),
                ]
            )
    ...
    def asymmetric(profiles, handle_missing="pair_delete", index_range=None):
        ...
        if handle_missing not in ("absolute_distance",):
            for i2, id in enumerate(np.arange(*index_range)):
                profile, presence = profiles[id], presences[id]
                diffs = (
                    np.sum(((profiles != profile) & presence), axis=1)
                    * float(presence.size)
                    / np.sum(presence)
                )
                distances[:, i2] = diffs
    ...
    def symmetric(profiles, handle_missing="pair_delete", index_range=None):
        ...
        if handle_missing in ("as_allele",):
            ...
            presences = (
                np.repeat(
                    np.sum(profiles > 0, 0) >= profiles.shape[0], profiles.shape[0]
                )
                .reshape([profiles.shape[1], profiles.shape[0]])
                .T
            )
        distances = np.zeros(
            shape=[profiles.shape[0], index_range[1] - index_range[0]], dtype=np.float32
        ...
        if handle_missing in ("pair_delete",):
            for i2, id in enumerate(np.arange(*index_range)):
                ...
                diffs = (
                    (np.sum((profiles[:id] != profile) & comparable, axis=1) + 0.01)
                    * float(presence.size)
                    / (np.sum(comparable, axis=1) + 0.01)
                )
            ...
            for i2, id in enumerate(np.arange(*index_range)):
                profile, presence = profiles[id], presences[id]
                diffs = np.sum(
                    (profiles[:id] != profile) & (presences[:id] * presence), axis=1
                )
    ...
    def symmetric_link(profiles, links, handle_missing="pair_delete"):
        ...
        return [
            [s, t, np.sum((profiles[s] != profiles[t]) & presences[s] & presences[t])]
            for s, t, d in links
        ]
    ...
    def eBurst(dist, n_str):
        weights = np.apply_along_axis(
            ...
            np.hstack(
                [
                    dist.astype(int),
                    np.array([[np.max(dist).astype(int) + 1]] * dist.shape[1]),
                ]
            ),
        )
        weights.T[0] += n_str
...
class methods(object):
    ...
    def _symmetric(dist, weight, **params):
        def minimum_spanning_tree(dist):
            ...
            edges = np.array(
                [[x, y, dist[x, y]] for y in np.arange(n_node) for x in np.arange(y)]
            )
    ...
    def _asymmetric(dist, weight, **params):
        ...
        try:
            ...
            with open(dist_file, "w") as fout:
                for d in dist:
                    fout.write(
                        "{0}\n".format(
                            "\t".join(
                                ["{0:.5f}".format(dd) for dd in (d + (1.0 - 0.000005))]
                            )
                        )
                    )
            del dist, d
            mstree = Popen(
                [params["edmonds_" + platform.system()], dist_file], stdout=PIPE
            ).communicate()[0]
            os.unlink(dist_file)
            ...
            return [
                [presence[d[0]], presence[d[1]], int(d[2]["weight"])]
                for d in ms.edges(data=True)
            ] + shortcuts.tolist()
    ...
    def _branch_recraft(branches, dist, weights, n_loci):
        ...
        branches = sorted(
            branches,
            key=lambda br: [dist[br[0], br[1]]]
            + sorted([weights[br[0]], weights[br[1]]]),
        )
        i = 0
        while i < len(branches):
            ...
            if len(sources) > 1:
                for w, d, s in sorted(
                    zip(weights[sources], dist[sources, tgt], sources)
                )[:3]:
                    if s == src:
                        break
                    if d < 1.5 * dist[src, tgt]:
                        if contemporary(
                            [dist[s, src], dist[src, s]], d, dist[src, tgt], n_loci
                        ):
                            tried[src], src = s, s
                            break
                while src not in tried:
                    ...
                    mid_nodes = sorted(
                        [
                            [weights[s], dist[s, tgt], s]
                            for s in childrens[src]
                            if s not in tried and dist[s, tgt] < 2 * dist[src, tgt]
                        ]
                    )
                    for w, d, s in mid_nodes:
                        if d < dist[src, tgt]:
                            if not contemporary(
                                [dist[src, s], dist[s, src]], dist[src, tgt], d, n_loci
                            ):
                                tried[src], src = s, s
                                break
                        elif w < weights[src]:
            ...
            if len(targets) > 1:
                for w, d, t in sorted(
                    zip(weights[targets], dist[src, targets], targets)
                )[:3]:
                    if t == tgt:
                        break
                    if d < 1.5 * dist[src, tgt]:
                        if contemporary(
                            [dist[t, tgt], dist[tgt, t]], d, dist[src, tgt], n_loci
                        ):
                            tried[tgt], tgt = t, t
                            break
                while tgt not in tried:
                    ...
                    mid_nodes = sorted(
                        [
                            [weights[t], dist[src, t], t]
                            for t in childrens[tgt]
                            if t not in tried and dist[src, t] < 2 * dist[src, tgt]
                        ]
                    )
                    for w, d, s in mid_nodes:
                        if d < dist[src, tgt]:
                            if not contemporary(
                                [dist[tgt, t], dist[t, tgt]], dist[src, tgt], d, n_loci
                            ):
                                tried[tgt], tgt = t, t
                                break
                        elif w < weights[tgt]:
    ...
    def MSTree(
        ...
        **params
        ...
        weight = eval("distance_matrix." + heuristic)(
            dist, [len(embeded[n]) for n in names]
        )

        tree = eval("methods._" + matrix_type)(dist, weight, **params)
        del dist
        if branch_recraft:
            tree = methods._branch_recraft(
                tree, np.load(params["dist_file"]), weight, n_loci
            )
        if matrix_type != "blockwise":
            tree = distance_matrix.symmetric_link(
                np.load(params["prof_file"]), tree, handle_missing=handle_missing
            )
    ...
    def goeBurst(names, profiles, embeded, handle_missing="pair_delete", **params):
        ...
        if handle_missing == "pair_delete":
            for n, p in enumerate(profiles):
                goeburst.stdin.write(
                    "{0}\t{1}\n".format(
                        n, "\t".join([str(pp) if pp > 0 else "-" for pp in p])
                    )
                )
        else:
            for n, p in enumerate(profiles):
        ...
        tree = distance_matrix.symmetric_link(
            profiles, tree, handle_missing=handle_missing
        )
        tree = methods._network2tree(tree, names)
        return tree
    ...
    def distance(
        ...
        **params
        ...
        for n, d in zip(names, dist):
            dist_txt.append(
                "{0!s:10} {1}".format(n, " ".join(["{:.6f}".format(dd) for dd in d]))
            )
        return dist_txt
    ...
    def fastme(names, profiles, embeded, handle_missing="pair_delete", **params):
        ...
        try:
            Popen(
                [
                    params["NJ_{0}".format(platform.system())],
                    "-i",
                    dist_file,
                    "-m",
                    "B",
            ...
            if platform.system() == "Linux":
                Popen(
                    [params["NJ_Linux32"], "-i", dist_file, "-m", "N"], stdout=PIPE
                ).communicate()
            else:
                raise e
    ...
    def ninja(names, profiles, embeded, handle_missing="pair_delete", **params):
        ...
        del dist, d
        free_memory = int(0.9 * psutil.virtual_memory().total / (1024.0**2))
        ninja_out = Popen(
            [
                "java",
                "-d64",
                "-Xmx" + str(free_memory) + "M",
                "-jar",
                params["ninja_{0}".format(platform.system())],
                "--in_type",
                "d",
                dist_file,
            ],
...
def nonredundant(names, profiles):
    encoded_profile = np.array(
        [np.unique(p, return_inverse=True)[1] + 1 for p in profiles.T]
    ).T
...
def backend(**args):
    ...
    try:
        if params["profile"][-3:].lower().endswith(".gz"):
            fin = (
                gzip.open(params["profile"], "rt").readlines()
                if os.path.isfile(params["profile"])
                else params["profile"].split("\n")
            )
        else:
            fin = (
                open(params["profile"]).readlines()
                if os.path.isfile(params["profile"])
                else params["profile"].split("\n")
            )
    except:
    ...
    for line_id, line in enumerate(fin):
        if line.startswith("#"):
            if not line.startswith("##"):
                ...
                allele_cols = np.array(
                    [
                        id
                        for id, col in enumerate(header)
                        if id > 0
                        and not col.startswith("#")
                        and not col.lower() in {"st_id", "st"}
                    ]
                )
            continue
        if line.startswith(">"):
    ...
    if int(params.get("checkEnv", False)):
        time, memory = estimate_Consumption(
            platform.system(),
            params["method"],
            params["matrix_type"],
            int(params["n_proc"]),
        ...
        return json.dumps(
            dict(time=time, memory=memory, affordable=free_memory >= memory)
        )
...
def estimate_Consumption(platform, method, matrix, n_proc, n_loci, n_profile):
    if method in ("MSTree", "RapidNJ", "ninja"):
        if matrix == "asymmetric":
            if platform == "Windows":
                time = (
                    5.600754e-6 * n_profile * n_profile
                    + 6.22306e-9 * n_loci * n_profile * n_profile / n_proc
                    + 22.71
                )
                memory = 182.16 * n_profile * n_profile + 282674000
            else:
                time = (
                    2.431284e-6 * n_profile * n_profile
                    + 2.701426667e-9 * n_loci * n_profile * n_profile / n_proc
                    + 33.753
                )
            ...
            if platform == "Windows":
                time = (
                    3.362214e-6 * n_profile * n_profile
                    + 3.735793333e-9 * n_loci * n_profile * n_profile / n_proc
                    + 20
                )
                memory = 70.140 * n_profile * n_profile + 292156000
            else:
                time = (
                    2.272428e-6 * n_profile * n_profile
                    + 32.625
                    + 2.52492e-9 * n_loci * n_profile * n_profile / n_proc
                )
        ...
        if platform == "Windows":
            ...
            memory = max(
                0.058292 * n_profile * n_profile * n_profile,
                1.39e6 * n_profile - 9.86e8,
            )
        else:
