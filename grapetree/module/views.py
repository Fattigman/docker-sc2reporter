...
app.wsgi_app = DispatcherMiddleware(
    Response("Not Found", status=404), {"/grapetree": app.wsgi_app}
)
...
def index():
    if request.method == "POST":
        ...
        return render_template(
            "MSTree_holder.html", version=app.config.get("VERSION"), data=data
        )
    else:
        return render_template(
            "MSTree_holder.html", version=app.config.get("VERSION"), data=None
        )
...
def sendToMicroReact(debug=None):
    ...
    try:
        if debug:
            ...
            tree, metaString, colors, name = (
                params["tree"][0],
                params["metadata"][0],
                json.loads(params["colors"][0]),
                params["name"][0],
            )

        metadata = pd.read_csv(
            StringIO(metaString), sep="\t", header=[0], dtype=str, na_filter=False
        )
        for fld, categories in colors.items():
            if not fld.endswith("__color"):
                metadata[fld + "__color"] = (
                    metadata[fld]
                    .astype(str)
                    .map(lambda v: categories.get(v, "#FFFFFF"))
                )
        ...
        geo_columns = list(
            filter(
                lambda c: re.match(
                    r"village|town|city|locat|state|region|district|^site$|geograph",
                    c.lower(),
                ),
                columns,
            )
        ...
        country_column = list(
            filter(lambda c: re.match(r"country", c.lower()), columns)
        )
        if len(geo_columns) or len(country_column):
            ...
            for index, strain in metadata.iterrows():
                if np.any(strain[["latitude", "longitude"]] == ""):
                    address = " ".join(
                        strain[geo_columns].values[strain[geo_columns] != ""]
                    )
                    country = (
                        "" if len(country_column) <= 0 else strain[country_column][0]
                    )
            ...
            for (_, _, index), geocode in zip(toRun, results):
                if geocode["Longitude"]:
                    metadata.at[index, "longitude"], metadata.at[index, "latitude"] = (
                        geocode["Longitude"],
                        geocode["Latitude"],
                    )

        column_ids = {
            c.lower(): id for id, c in reversed(list(enumerate(metadata.columns)))
        }
        ...
        q = requests.post(
            ...
            json=dict(tree=tree, data=metaString, name=name),
        )
