...
for name, group in x:
    ...
    agg_b = df_sel_b.groupby(
        pd.cut(df_sel_b[0], (0, 0.00008, 0.0008, 0.008, 0.08, 0.8))
    ).aggregate(np.mean)
    agg_u = df_sel_u.groupby(
        pd.cut(df_sel_u[0], (0, 0.00008, 0.0008, 0.008, 0.08, 0.8))
    ).aggregate(np.mean)
    ...
    cax = ax1.scatter(
        agg_b[4], agg_b[9], cmap=mpl.cm.Greys, norm=norm, c=agg_b[5], zorder=0, s=0
    )
