...
for name, group in x:
    ...
    agg = df_sel.groupby(
        pd.cut(df_sel["rate"], (0, 1600, 3200, 4800, 6400, 8001))
    ).aggregate(np.mean)
    cmap = colors[counter]
    norm = mpl.colors.Normalize(vmin=0.00001, vmax=8500)
    # fake plot for colorbar
    cax = ax.scatter(
        agg[3], agg[7], cmap=mpl.cm.Greys, norm=norm, c=agg[4], zorder=0, s=0
    )
...
for name, group in x:
    ...
    agg = df_sel.groupby(pd.cut(df_sel["rate"], (0, 8, 16, 24, 32, 41))).aggregate(
        np.mean
    )
    cmap = colors[counter]
...
for name, group in x:
    ...
    agg = df_sel.groupby(
        pd.cut(df_sel["rate"], (0, 400, 800, 1200, 1600, 2000))
    ).aggregate(np.mean)
    cmap = colors[counter]
