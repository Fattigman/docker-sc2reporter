...
for name, group in x:
    ...
    agg_b = df_sel_b.groupby(pd.cut(df_sel_b[1], (0, 500, 1000, 1500, 2001))).aggregate(
        np.mean
    )
    agg_u = df_sel_u.groupby(pd.cut(df_sel_u[1], (0, 500, 1000, 1500, 2001))).aggregate(
        np.mean
    )
    cmap = colors[counter]
    ...
    cax = ax1.scatter(
        agg_b[4], agg_b[9], cmap=mpl.cm.Greys, norm=norm, c=agg_b[6], zorder=0, s=0
    )
